...
function CheckList() {
  ...
  const [goals, setGoals] = useState(() => {
    ...
    try {
      ...
      return savedGoals
        ? JSON.parse(savedGoals).map((goal) => ({
            ...goal,
      ...
      console.error(
        "Failed to load goals from localStorage (fallback):",
        error
      );
      return [];
    }
    ...
    const newGoalObject = {
      ...
      frequency:
        newGoalFrequency === "daily"
          ? { type: "daily" }
          : { type: "custom", details: { days: customFrequencyDetails } },
      history: [],
    };
      ...
      goal.id === id
        ? {
            ...
            history: goal.history.includes(today)
              ...
              : [...goal.history, today], // Add/remove today's date
          }
        : goal
    );
  ...
  const getNextScheduledDay = (currentDay, scheduledDays) => {
    ...
    const scheduledDayIndices = scheduledDays
  ...
  const calculateStreak = (goal) => {
    ...
    const sortedHistory = [...goal.history]
      ...
      .sort((a, b) => a - b); // Sort dates ascending
    let currentStreak = 0;
    ...
    if (mostRecentDateString === todayString) {
      ...
      if (
        ...
        mostRecentDateString === yesterdayString
      ) {
        currentStreak = 1;
      } else if (
        goal.frequency.type === "custom" &&
        goal.frequency.details &&
        goal.frequency.details.days
      ) {
        ...
        const nextScheduledDayAfterLastCompletion = getNextScheduledDay(
          lastCompletionDayOfWeek,
          goal.frequency.details.days
        );
    ...
    for (let i = sortedHistory.length - 2; i >= 0; i--) {
      ...
      if (goal.frequency.type === "daily") {
        const timeDiff = Math.abs(
          completionDate.getTime() - previousCompletionDate.getTime()
        );
        ...
        const nextScheduledDayAfterCurrent = getNextScheduledDay(
          currentDayOfWeek,
          goal.frequency.details.days
        );

        if (
          isScheduledDay(completionDate, goal.frequency.details.days) &&
          nextScheduledDayAfterCurrent === previousDayOfWeek
        ) {
          currentStreak++;
        } else {
          break; // Gap in streak
